...
export type FlowData = MemoryNode & { onOpenMedia?: (assets: Asset[], index: number) => void } & Record<string, unknown>
...
export default function MemoryCard({ data, selected, id }: NodeProps<FlowMemoryNode>) {
  ...
  const showHandles = selected || hasConnections
  useEffect(() => { updateNodeInternals(id) }, [id, playlistOpen, updateNodeInternals])
  return <div className={`memory-card ${node.type} ${largeCover ? 'music-large' : ''} ${selected ? 'selected' : ''}`}>
    ...
    {showHandles && <Handle type="target" position={Position.Left} style={{ top: '50%', transform: 'translateY(-50%)' }} />}
    ...
    {showHandles && <Handle type="source" position={Position.Right} style={{ top: '50%', transform: 'translateY(-50%)' }} />}
  </div>
}
