...
const emptyTrack: Track = { title: '', artist: '', kind: 'track', cover_size: 'small', playlist_items: [], collapsed_item_limit: 3 }
...
export default function Editor({ node, boardStartDate, boardEndDate, onClose, onSave, onRequestDelete, onDeleteAsset, onUpdateAsset, onReorderAssets, onPreview }: Props) {
  ...
  const selectSpotifyTrack = (result: SpotifyTrack) => {
    updateDraft(current => {
      ...
      return { ...current, track_data: { ...track, title: result.title, artist: result.artist, spotify_id: result.id, spotify_cover_url: result.cover_url || null } }
    })
  ...
  return <aside className="editor" onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget as Node)) void save() }} onPointerDown={event => event.stopPropagation()} onKeyDown={event => event.stopPropagation()} onKeyUp={event => event.stopPropagation()}>
    ...
    {node.type === 'track' && <><label>Тип музыки<select value={track.kind} onChange={event => updateTrack('kind', event.target.value as Track['kind'])}><option value="track">Трек</option><option value="playlist">Плейлист</option></select></label>{track.kind === 'track' ? <><label>Название трека<input value={track.title} onChange={event => updateTrack('title', event.target.value)} /></label><label>Исполнитель<input value={track.artist} onChange={event => updateTrack('artist', event.target.value)} /></label>{spotifySearch}{track.title && <p className="spotify-selected">Выбрано: <strong>{track.title}</strong>{track.artist && ` — ${track.artist}`}</p>}</> : <><label>Название плейлиста<input value={track.title} onChange={event => updateTrack('title', event.target.value)} placeholder="Можно оставить пустым" /></label>{spotifySearch}<label>Треков в свёрнутом виде<input type="number" min="0" max="10" value={track.collapsed_item_limit} onChange={event => updateTrack('collapsed_item_limit', Math.max(0, Math.min(10, Number(event.target.value) || 0)))} /></label><div className="playlist-editor"><p>Треки в плейлисте</p>{track.playlist_items.map((item, index) => <div className={`playlist-row ${draggedPlaylistIndex === index ? 'dragging' : ''} ${playlistDropIndex === index && draggedPlaylistIndex !== index ? 'drop-target' : ''}`} key={`${item.title}-${item.artist}-${index}`} draggable onDragStart={() => setDraggedPlaylistIndex(index)} onDragEnd={() => { setDraggedPlaylistIndex(null); setPlaylistDropIndex(null) }} onDragOver={event => { event.preventDefault(); setPlaylistDropIndex(index) }} onDragLeave={() => setPlaylistDropIndex(current => current === index ? null : current)} onDrop={() => { if (draggedPlaylistIndex !== null && draggedPlaylistIndex !== index) reorderPlaylistItems(draggedPlaylistIndex, index); setDraggedPlaylistIndex(null); setPlaylistDropIndex(null) }}><span className="playlist-drag" title="Перетащите для изменения порядка">⠿</span><button type="button" className={`playlist-favorite ${item.is_favorite ? 'active' : ''}`} title="Показывать в свёрнутом виде" onClick={() => updatePlaylistItem(index, 'is_favorite', !item.is_favorite)}>★</button><input value={item.title} placeholder="Название" onChange={event => updatePlaylistItem(index, 'title', event.target.value)} /><input value={item.artist} placeholder="Исполнитель" onChange={event => updatePlaylistItem(index, 'artist', event.target.value)} /><button type="button" className="remove-playlist-item" onClick={() => updateDraft(current => ({ ...current, track_data: { ...track, playlist_items: track.playlist_items.filter((_, itemIndex) => itemIndex !== index) } }))}>×</button></div>)}<button type="button" className="add-track" onClick={() => updateDraft(current => ({ ...current, track_data: { ...track, playlist_items: [...track.playlist_items, { title: '', artist: '', cover_url: null, is_favorite: false }] } }))}>+ Добавить трек вручную</button></div></>}<label>Иконка<select value={track.cover_size} onChange={event => updateTrack('cover_size', event.target.value as Track['cover_size'])}><option value="small">Маленькая</option><option value="large">Большая</option></select></label>{track.kind !== 'playlist' && !track.cover_url && track.spotify_cover_url && <p className="spotify-cover-hint">Обложка получена из Spotify</p>}<label>Обложка URL (вручную)<input value={track.cover_url || ''} onChange={event => updateTrack('cover_url', event.target.value)} /></label></>}
    <label className="toggle-label"><input type="checkbox" checked={draft.show_type_label ?? false} onChange={event => updateDraft({ show_type_label: event.target.checked })} />Показывать тип ноды</label><label>Дата в периоде<input type="date" min={boardStartDate} max={boardEndDate} value={draft.temporal_date || ''} onChange={event => updateDraft({ temporal_date: event.target.value || null })} /></label><label>Расположение даты<select value={draft.date_position || 'bottom-center'} onChange={event => updateDraft({ date_position: event.target.value as DatePosition })}><option value="top-left">Слева сверху</option><option value="top-center">По центру сверху</option><option value="top-right">Справа сверху</option><option value="bottom-left">Слева снизу</option><option value="bottom-center">По центру снизу</option><option value="bottom-right">Справа снизу</option></select></label>{error && <p className="error">{error}</p>}<div className="editor-actions"><button className="danger" onClick={onRequestDelete}>Удалить</button>{busy && <span className="saving">Сохраняю…</span>}</div>
  </aside>
}
